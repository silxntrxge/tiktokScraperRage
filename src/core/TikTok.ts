...
export class TikTokScraper extends EventEmitter {
    ...
    public async scrape(action?: string, params?: any): Promise<Result | any> {
        try {
            await this.checkTikTokAvailability();

            if (action) {
                switch (action) {
                    case 'scrape_video':
                        return await this.scrapeVideo(params.url);
                    case 'scrape_music':
                        return await this.scrapeMusic(params.musicId, params.count);
                    default:
    ...
    private async submitScrapingRequest(query: RequestQuery, updatedApiResponse = false): Promise<boolean> {
        try {
            console.log('Submitting scraping request with query:', JSON.stringify(query));

            if (!this.validHeaders) {
                if (this.scrapeType === 'trend') {
                    console.log('Getting valid headers for trend scraping');
                    await this.getValidHeaders(`https://www.tiktok.com/foryou`, false, 'GET');
                }
                this.validHeaders = true;
            }

            console.log('Scraping data with query');
            ...
            if ((updatedApiResponse && !result.itemList) || (!updatedApiResponse && !result.items)) {
                console.log('No items found in the API response. Falling back to web scraping.');
                const webScrapedData = await this.scrapeDataFromWeb();
                if (webScrapedData.length > 0) {
                    await this.collectPosts(webScrapedData);
                    return true;
                }
            ...
            if (!hasMore) {
                console.log(`Only ${this.collector.length} results could be found.`);
                return true;
            }
            ...
            console.error('Error in submitScrapingRequest:', error);
            throw error;
        }
    }
    ...
    private async getVideoMetadataFromHtml(): Promise<FeedItems> {
        ...
        try {
            ...
            if (response.includes('SIGI_STATE')) {
                ...
                const videoProps = JSON.parse(rawVideoMetadata);
                const firstItem = Object.values(videoProps.ItemModule)[0] as any;
                return firstItem;
            }
    ...
    async getUserData(username: string) {
        const root = await loadProtoFile('path/to/tiktok.proto');
        const payload = {
            username: username,
            // ... other required fields ...
        };
        const encodedMessage = encodeMessage(root, 'UserRequest', payload);
        
        const response = await this.request({
            url: 'https://im-api.tiktok.com/v1/message/get_by_user',
            method: 'POST',
            body: encodedMessage,
            headers: {
        ...
        const decodedMessage = decodeMessage(root, 'UserResponse', response.data);
        return decodedMessage;
    }

    private async checkTikTokAvailability(): Promise<void> {
        try {
            const response = await this.request<string>({
                uri: 'https://www.tiktok.com',
                method: 'GET',
            });
            if (response.includes('tiktok-verify-page')) {
    ...
    private async scrapeVideo(url: string): Promise<PostCollector> {
        this.input = url;
        return await this.getVideoMeta(true);
    }

    private async scrapeMusic(musicId: string, count: number): Promise<Result> {
        this.input = musicId;
        this.number = count;
        this.scrapeType = 'music';
    ...
    private async scrapeDataFromWeb(): Promise<any[]> {
        try {
            const response = await this.request<string>({
            ...
            // This is a placeholder and needs to be implemented based on the current structure of TikTok's web page
            return [];
        } catch (error) {
