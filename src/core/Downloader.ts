...
export class Downloader {
    ...
    async getTikTokUserMusicLinks(username: string): Promise<string[]> {
        ...
        for (const videoLink of videoLinks) {
            const musicUrl = await this.extractMusicUrlFromVideo(videoLink);
            if (musicUrl) {
                musicLinks.push(musicUrl);
            }
        }
    ...
    private async extractMusicUrlFromVideo(videoUrl: string): Promise<string | null> {
        const response = await axios.get(videoUrl);
        const $ = cheerio.load(response.data);

        const musicElement = $('a[href^="/music/"]');
        const musicHref = musicElement.attr('href');

        if (musicHref) {
            return `https://www.tiktok.com${musicHref}`;
        }
